...
public class VideoGameLevel {
    ...
    public static void main(String[] argv) {
        ...
        VideoGameLevel obj = new VideoGameLevel();
        System.out.println(obj.level(obstacles_1, instructions_1));
        System.out.println(obj.level(obstacles_1, instructions_2));
        System.out.println(obj.level(obstacles_1, instructions_3));
        System.out.println(obj.level(obstacles_1, instructions_4));
        System.out.println(obj.level(obstacles_1, instructions_5));
        System.out.println(obj.level(obstacles_1, instructions_6));
        System.out.println(obj.level(obstacles_1, instructions_7));
        System.out.println(obj.level(obstacles_1, instructions_8));
        System.out.println(obj.level(obstacles_1, instructions_9));
        System.out.println(obj.level(obstacles_1, instructions_10));
        System.out.println(obj.level(obstacles_2, instructions_11));
        System.out.println(obj.level(obstacles_3, instructions_9));
    }

    public boolean level(int[] obstacle, String instruction) {

        int position = 0;
        int direction = 0; //direction -> 0: right, 1: left

        Set<Integer> obstacleList = Arrays.stream(obstacle).boxed().collect(Collectors.toSet());

        for(char ch : instruction.toCharArray()) {
            if (direction == 0) {
                switch (ch) {
                    case 'R': position++; break;
                    case 'L': position--; direction = 1; break;
                    case 'J': position += 2;
                }
            } else {
                switch (ch) {
                    case 'R': position++; direction = 0; break;
            ...
            if (obstacleList.contains(position)) {
                return false;
            }
        }
        return false;
